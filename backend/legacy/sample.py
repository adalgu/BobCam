...
while True:
    ...
    if len(faces) > 0:
        ...
        if len(mar_history) == 60:
            ...
            # 입을 벌렸다 닫는 행동 감지
            if mar_std > 0.04 and mar_range > 0.15:
                eating_counter += 1
                not_eating_counter = 0
            else:
            ...
            if eating_counter > 5:  # 1초 이상 먹는 행동이 감지되면
                eating_detected = True
            elif not_eating_counter > 5:  # 3초 이상 먹는 행동이 감지되지 않으면
                eating_detected = False

            if eating_detected:
                message = "착하네~ 밥 잘 먹고 있어요"
                color = (0, 255, 0)
            else:
                message = "어서 밥 먹어요~"
                color = (0, 0, 255)
        ...
        frame = put_korean_text(frame, f"MAR: {mar:.2f}", (10, 30), font, (255, 0, 0))
        frame = put_korean_text(
            frame, f"Eating Counter: {eating_counter}", (10, 70), font, (255, 0, 0)
        )
        frame = put_korean_text(
            frame,
            f"Not Eating Counter: {not_eating_counter}",
            (10, 110),
            font,
            (255, 0, 0),
        )
