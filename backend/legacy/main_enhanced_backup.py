...
try:
    ...
    print("✅ 고급 턱 감지 모듈 로드 성공")
except ImportError:
    ADVANCED_DETECTION_AVAILABLE = False
    print("⚠️ 고급 감지 모듈 없음 - 기본 모드로 실행")


class EatingStatus:
    """
    기존 식사 상태를 감지하고 관리하는 클래스.
...
class EatingMonitorApp:
    def __init__(self, root):
        ...
        # 모듈 설정 (기본 vs 고급)
        self.use_advanced_detection = False  # 기본값은 False
        self.eating_status = EatingStatus()  # 기본 감지 시작
    ...
    def setup_detection_mode_selector(self):
        """감지 모드 선택 UI 설정"""
        mode_label = tk.Label(
    ...
    def on_detection_mode_change(self):
        ...
        print(f"감지 모드 변경: {'고급' if self.use_advanced_detection else '기본'}")
    ...
    def create_status_icons(self, parent_frame):
        ...
        # 기본 아이콘
        basic_icons = {"face": "얼굴", "mouth": "입 상태", "move": "움직임", "eat": "식사"}
        
        # 고급 모드에서 추가 아이콘
        if self.use_advanced_detection:
            icons = {**basic_icons, "jaw": "턱", "rhythm": "리듬"}
        else:
            icons = basic_icons

        # 아이콘 프레임들 생성
        for i, (name, text) in enumerate(icons.items()):
            ...
            if len(icons) <= 4:
                ...
                if i < 3:
                    ...
                    # 두 번째 줄을 위한 별도 프레임 필요 (여기서는 간단히 처리)
                    frame.pack(side=tk.LEFT, expand=True, fill=tk.BOTH)
        ...
        try:
            ...
            for point_idx in jaw_points:
                if point_idx < len(landmarks):
                    ...
                    x = int(landmark.x * w)
                    y = int(landmark.y * h)
                    cv2.circle(frame, (x, y), 2, (255, 0, 255), -1)  # 보라색 점
            ...
            for point_idx in chin_points:
                if point_idx < len(landmarks):
                    ...
                    x = int(landmark.x * w)
                    y = int(landmark.y * h)
                    cv2.circle(frame, (x, y), 3, (0, 255, 255), -1)  # 노란색 큰 점
    ...
    def update_status_icons(self, face_detected, states):
        ...
        if self.use_advanced_detection:
            # 턱 상태
            if "jaw" in self.status_labels:
                if states.get("jaw_state") == "움직임":
                    self.status_labels["jaw"].config(text="🦴", fg="blue")
                else:
                    self.status_labels["jaw"].config(text="🦴", fg="gray")
            
            # 리듬 상태 (씹기 패턴)
            if "rhythm" in self.status_labels:
                rhythm_score = states.get("chewing_rhythm_score", 0.0)
                if rhythm_score > 0.3:
                    self.status_labels["rhythm"].config(text="🎵", fg="green")
                elif rhythm_score > 0.1:
                    self.status_labels["rhythm"].config(text="🎵", fg="orange") 
                else:
                    self.status_labels["rhythm"].config(text="🎵", fg="gray")
